    ...
    <div class="container">
      <div class="user-manager-container">
        ...
        <div class="feedback-section mb-3">
          ...
          <div v-if="error && !showModal" class="alert alert-danger alert-dismissible fade show shadow-sm"><i class="fas fa-exclamation-triangle me-2"></i> <span v-html="error"></span> <button type="button" class="btn-close btn-sm" @click="clearError"></button></div>
        ...
        <div v-if="viewMode === 'active'" class="table-container shadow-sm mb-4">
          <div class="table-responsive">
            <table class="table table-hover align-middle modern-table">
              ...
              <tbody>
                <tr v-if="isLoading"><td :colspan="7" class="text-center"><div class="d-flex justify-content-center align-items-center py-4"><div class="spinner-border text-primary"></div><span class="ms-3 text-muted">Memuat...</span></div></td></tr>
                <tr v-for="(user, index) in users" :key="user.uuid" v-show="!isLoading">
                  ...
                  <td>
                    <div class="action-buttons">
                      ...
                      <!-- Disable delete button if it's the currently logged-in user -->
                      <button @click="confirmDeleteUser(user.uuid, user.name)" class="btn btn-icon btn-outline-danger" title="Hapus User (Trash)" :disabled="actionInProgress(user.uuid) || isCurrentUser(user.uuid)">
                 ...
                 <tr v-for="(user, index) in trashedUsers" :key="user.uuid" v-show="!isLoading">
                   ...
                   <td>
                     <div class="action-buttons">
                       ...
                       <!-- Disable force delete button if it's the currently logged-in user (though they shouldn't be in trash) -->
                       <button @click="confirmForceDeleteUser(user.uuid, user.name)" class="btn btn-icon btn-outline-danger" title="Hapus Permanen" :disabled="actionInProgress(user.uuid) || isCurrentUser(user.uuid)">
            ...
            <div class="modal-content shadow-lg">
              ...
              <div class="modal-footer">
                ...
                <button type="submit" form="userForm" class="btn btn-primary"
                        :disabled="isSaving || !currentUser.name || !currentUser.email || !currentUser.role_id || (!isEditing && (!currentUser.password || !currentUser.password_confirmation)) || (isEditing && currentUser.password && !currentUser.password_confirmation)">
...
apiClient.interceptors.request.use(config => {
  try { const userDataString = localStorage.getItem('userData'); if (userDataString) { const userData = JSON.parse(userDataString); const token = userData?.token; if (token) { config.headers = config.headers || {}; config.headers['Authorization'] = `Bearer ${token}`; } } } catch (e) { console.error("Error reading localStorage:", e); } return config;
}, error => Promise.reject(error));
apiClient.interceptors.response.use(response => response, error => {
    // Check specifically for 401 or 403 on non-GET requests
    if (error.response && (error.response.status === 401 || error.response.status === 403) && error.config.method !== 'get') {
        console.error(`Authorization Error (${error.response.status}) on ${error.config.method} ${error.config.url}`);
        localStorage.removeItem('userData');
        // Avoid immediate redirect if component handles error display
        // window.location.href = '/login';
        return Promise.reject(new Error(`Auth failed or forbidden: ${error.response.status}`)); // Reject to allow component catch
    }
    // Keep other console warnings for debugging non-auth errors
...
const getErrorMessage = (error, defaultMessage = 'Terjadi kesalahan.') => {
    // Check if it's the specific auth error we threw in the interceptor
...
const fetchLoggedInUser = () => {
     try {
         ...
         if (userDataString) {
            ...
            // console.log('Fetched Logged In User ID/UUID:', loggedInUserId.value);
         } else {
             console.warn('No user data found in localStorage for ID check.');
...
// --- API Functions ---
// fetchUsers, fetchTrashedUsers, fetchTrashCount - remain the same as previous correct version
...
async function fetchRoleOptions() { isLoadingRoleOptions.value = true; try { const response = await apiClient.get('/role-options'); roleOptions.value = response.data || []; } catch (err) { modalError.value = getErrorMessage(err, "Gagal memuat opsi role."); roleOptions.value = []; } finally { isLoadingRoleOptions.value = false; } }
...
async function saveUser() {
    ...
    if (currentUser.password) {
        if (!currentUser.password_confirmation) { modalError.value = 'Konfirmasi Password Baru wajib diisi jika ingin mengubah password.'; isSaving.value = false; return; }
        if (currentUser.password !== currentUser.password_confirmation) { modalError.value = 'Password dan Konfirmasi Password tidak cocok.'; isSaving.value = false; return; }
        payload.password = currentUser.password;
        payload.password_confirmation = currentUser.password_confirmation;
    } else if (!isEditing.value) { // Password required for creation
         modalError.value = 'Password dan Konfirmasi Password wajib diisi untuk user baru.'; isSaving.value = false; return;
    }
    ...
    try {
        ...
        if ((response.status === 200 || response.status === 201) && response.data?.data) { setSuccessMessage(`User "${response.data.data.name}" berhasil ${isEditing.value ? 'diperbarui' : 'ditambahkan'}.`); closeAndResetModal(); await fetchUsers(); } else { throw new Error(response.data?.message || `Gagal ${isEditing.value ? 'menyimpan' : 'membuat'} user.`); }
    } catch (err) { modalError.value = getErrorMessage(err, `Gagal ${isEditing.value ? 'menyimpan' : 'membuat'} user.`); } finally { isSaving.value = false; }
}
...
async function deleteUser(uuid, name) {
    ...
    try {
        ...
        if (response.status === 204) {
            ...
            await fetchUsers(); // Refreshes active list & trash count
        } else {
            console.warn("Unexpected status code for user delete:", response.status, response.data);
            throw new Error(response.data?.message || 'Gagal menghapus user.');
        }
    } catch (err) {
        // Handle specific auth errors potentially, otherwise show generic
        if (err.message?.includes('Auth failed or forbidden')) {
             error.value = 'Sesi Anda mungkin telah berakhir atau Anda tidak diizinkan menghapus user ini. Silakan login kembali.';
        } else if (err.response?.status !== 401 && err.response?.status !== 403) { // Avoid double message if interceptor handles it
...
async function restoreUser(uuid, name) {
    ...
    try {
        const response = await apiClient.post(`/users/restore/${uuid}`); if (response.status === 200 && response.data?.message) { setSuccessMessage(response.data.message || `User "${name}" berhasil dipulihkan.`); if (trashedUsers.value.length === 1 && trashedPagination.value.currentPage > 1) { trashedPagination.value.currentPage--; } await fetchTrashedUsers(); } else { throw new Error(response.data?.message || `Gagal memulihkan user "${name}".`); }
    } catch (err) { error.value = getErrorMessage(err, `Gagal memulihkan user "${name}".`); } finally { restoringId.value = null; }
}
...
async function forceDeleteUser(uuid, name) {
    ...
    forceDeletingId.value = uuid; error.value = null; successMessage.value = null; const indexToDelete = trashedUsers.value.findIndex(u => u.uuid === uuid);
    try {
        const response = await apiClient.delete(`/users/force-delete/${uuid}`); if (response.status === 204) { setSuccessMessage(`User "${name}" berhasil dihapus permanen.`); if (indexToDelete > -1) { trashedUsers.value.splice(indexToDelete, 1); } const needsPageDecrement = trashedUsers.value.length === 0 && trashedPagination.value.currentPage > 1; if (needsPageDecrement) { trashedPagination.value.currentPage--; } await fetchTrashedUsers(); } else { throw new Error(response.data?.message || `Gagal menghapus user "${name}" permanen.`); }
    } catch (err) {
         if (err.message?.includes('Auth failed or forbidden')) {
              error.value = 'Sesi Anda mungkin telah berakhir atau Anda tidak diizinkan menghapus user ini. Silakan login kembali.';
...
// --- UI Functions ---
// applyFilters, clearSearch, clearAllFilters, clearError, setSuccessMessage, changePage, changeSort - remain the same
function applyFilters() { if (viewMode.value === 'active') { pagination.value.currentPage = 1; fetchUsers(); } else { trashedPagination.value.currentPage = 1; fetchTrashedUsers(); } }
...
function changePage(page) { if (isLoading.value) return; if (viewMode.value === 'active') { if (page >= 1 && page <= pagination.value.totalPages && page !== pagination.value.currentPage) { pagination.value.currentPage = page; fetchUsers(); } } else { if (page >= 1 && page <= trashedPagination.value.totalPages && page !== trashedPagination.value.currentPage) { trashedPagination.value.currentPage = page; fetchTrashedUsers(); } } }
function changeSort(column) { if (viewMode.value !== 'active' || isLoading.value) return; if (filters.sort_by === column) { filters.sort_direction = filters.sort_direction === 'asc' ? 'desc' : 'asc'; } else { filters.sort_by = column; filters.sort_direction = 'asc'; } applyFilters(); }
...
function openAddModal() { isEditing.value = false; modalError.value = null; resetCurrentUserForm(); fetchRoleOptions(); showModal.value = true; }
function openEditModal(user) { isEditing.value = true; modalError.value = null; Object.assign(currentUser, { uuid: user.uuid, name: user.name, email: user.email, role_id: user.role_id, is_active: user.is_active, password: '', password_confirmation: '' }); fetchRoleOptions(); showModal.value = true; }
function closeAndResetModal() { showModal.value = false; isEditing.value = false; modalError.value = null; resetCurrentUserForm(); }
function confirmDeleteUser(uuid, name) { if (isCurrentUser(uuid)) { error.value="Aksi tidak diizinkan."; return; } if (confirm(`Yakin ingin memindahkan user "${name}" ke tempat sampah?`)) { deleteUser(uuid, name); } }
function confirmForceDeleteUser(uuid, name) { if (isCurrentUser(uuid)) { error.value="Aksi tidak diizinkan."; return; } if (confirm(`PERHATIAN! Yakin hapus permanen user "${name}"? Aksi ini tidak dapat diurungkan.`)) { forceDeleteUser(uuid, name); } }
function switchViewMode(mode) { if (mode === viewMode.value || isLoading.value) return; viewMode.value = mode; clearError(); successMessage.value = null; if (mode === 'active') { pagination.value.currentPage = 1; fetchUsers(); } else { trashedPagination.value.currentPage = 1; fetchTrashedUsers(); } }
...
// --- Lifecycle Hook ---
onMounted(() => { fetchLoggedInUser(); fetchUsers(); fetchRoleOptions(); });
